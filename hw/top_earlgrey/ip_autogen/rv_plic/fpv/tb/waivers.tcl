...
# This assertion states that if the input enable (en_i) is not high and onehot input (oh_i) is true
# then we get an output error (err_o).
...
# u_prim_onehot_check should get asserted. But oh_i can always be 1'b1 or 1'b0. The reason is the
# same argued above. Due to the fact that oh_i is wired to reg_we_check in u_reg and each bit of
# reg_we_check is driven by a range of software controlled registers. Those registers are a single
# bit value and which is true if their respective addr_hit[x] is true along with reg_we and
# !reg_error. But addr_hit is also a onehot0 vector as each bit of addr_hit represents a specific
# address hit of a register. The address is coming from reg_addr as an output from u_reg_if and
# that can only have an address of a single software controlled register. That address is checked
# against the adrress of every register and whichever matches, the corresponding addr_hit bit gets
# updated to 1.
# To conclude this, oh_i means that we have a mapped register address with a reg write enable
# without a register error. Since a register could be a mapped one or not, this assertion is not
# possible to cover.
